#ifndef ODE_SOLVER_HEADER_H
#define ODE_SOLVER_HEADER_H
...
int f(State *restrict x, double dt) {
	...
	double I_HCN_den = x->g_HCN_den * x->m_HCN_den * (x->V_d - E_HCN); // HCN current
	...
	double I_chi_den = chi_den * (x->g_GABA_den + g_ton_den) * (x->V_s - E_Cl_den);
